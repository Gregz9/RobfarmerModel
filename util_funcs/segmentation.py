...
sam2_checkpoint = "/app/data/sam_ckpts/sam2.1_hiera_large.pt"
...
def automatic_masks(
    img: np.ndarray, checkpoint: str = sam2_checkpoint, config: str = model_cfg
):
...
if __name__ == "__main__":
    ...
    img_display = cv.resize(
        cv.cvtColor(img.copy(), cv.COLOR_RGB2BGR),
        (900, 700),
    ...
    cv.setMouseCallback(
        "Segmentation point choice", mouse_callback_left, (points, img_display)
    )
