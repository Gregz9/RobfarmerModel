...
from interaction_hotspots.utils.evaluation import AUC_Judd
...
class IntCAM(nn.Module):
    ...
    def backward_for_target_grad(self, output, activations, target_grad):
        ...
        cam = F.relu(self.gradients * activations).sum(
            2
        )  # (B, T, C, 7, 7) --> (B, T, 7, 7)
    ...
    def generate_cams(self, inputs, classes, **kwargs):
        ...
        for c_id in classes:
            ...
            cam = self.backward_for_target_grad(
                outputs, activations, target_grad
            )  # (B, T, 7, 7) for a single class
            cams.append(cam.cpu())
