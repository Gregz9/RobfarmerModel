...
# In-place computations directly modify the contents of tensors,
# leading to the sometimes undesired behavior of changing values
# before they are used. By default, ReLU is initiated with inplace=True
def replace_relu(model):

    for name, child in model.names_children():
...
def save_activations(activations, module, input, output):
...
class FrameLSTM(nn.Module):
    ...
    def __init__(self, num_classes, max_len, hidden_size, pool_fn="L2", ant_loss=None):
        ...
        # GradCAM specific attributes
        self.gradients = None
        self.activations = None

    def hook_grad(self, grad):
        """Hook function to capture gradients for GradCAM"""
        self.gradients = grad

    def hook_activation(self, module, input, output):
        """Hook function to capture activations for GradCAM"""
    ...
    def generate_gradcam(self, frame_feats, preds, target_classes):
        """
        Generate GradCAM using gradient-weighted class activation mapping
        Args:
            frame_feats: Feature maps from backbone (B*T, C, H, W) - same as fmaps in cons_rnn
            preds: Predictions from classifier (B, num_classes)
            target_classes: Target class indices (B*T,)
        Returns:
            cams: Class activation maps (B*T, H, W)
        """
        B_T, C, H, W = frame_feats.shape
        B = preds.shape[0]
        T = B_T // B

        # Store activations for GradCAM
        self.activations = frame_feats

        # Register hook to capture gradients
        hook_handle = frame_feats.register_hook(self.hook_grad)

        # Get target class scores from the auxiliary predictions that maintain gradients
        target_classes_batch = target_classes[
            :B
        ]  # Take first B classes (one per batch item)
        class_scores = preds[
            torch.arange(B), target_classes_batch
        ]  # (B,) - preds here is aux_preds from caller

        # Backward pass to compute gradients w.r.t. feature maps
        # Now gradients should flow back to fmaps through the auxiliary prediction path
        class_scores.sum().backward(retain_graph=True)

        # Get the captured gradients
        gradients = self.gradients

        # Check if gradients is None
        if gradients is None:
            # Return zero CAMs if gradients are None
            print("GRADIENT IS FUCKING NONE MAN")
            hook_handle.remove()
            self.gradients = None
            self.activations = None
            return torch.zeros(B_T, H, W, device=frame_feats.device)

        # GradCAM computation
        # Global average pooling of gradients to get importance weights
        weights = F.adaptive_avg_pool2d(gradients, (1, 1))  # (B*T, C, 1, 1)

        # Weighted combination of activation maps
        cams = (weights * self.activations).sum(dim=1, keepdim=False)  # (B*T, H, W)
    ...
    def forward(self, batch, **kwargs):
        ...
        # Ensure fmaps requires gradients for GradCAM
        fmaps = fmaps.requires_grad_(True)
        ...
        # MODIFIED: Create auxiliary prediction path for GradCAM that maintains gradients
        # We need to create a computational path from fmaps to predictions
        # Use Global Average Pooling on fmaps to create frame-level features
        fmaps_pooled = F.adaptive_avg_pool2d(fmaps, (1, 1)).view(B * T, -1)  # (B*T, C)

        # Reshape to (B, T, C) and pass through LSTM to get auxiliary predictions
        fmaps_pooled_seq = fmaps_pooled.view(B, T, -1)
        aux_clip_feats, _ = self.embed_clip(fmaps_pooled_seq, length=length)
        aux_preds = self.fc(
            aux_clip_feats
        )  # Auxiliary predictions with gradient connectivity

        # Extract the correct class labels for CAM generation
        labels = torch.repeat_interleave(torch.argmax(preds, dim=1), T)

        # Generate GradCAMs using auxiliary predictions that maintain gradient connectivity
        cams = self.generate_gradcam(fmaps, aux_preds, labels)  # (B*T, H, W)

        # Reshape and refine CAMs
        cams = cams.unsqueeze(1)  # Add channel dimension for refine_cams
        cams = self.refine_cams(cams, image_shape=gazemaps.shape[3:])
        cams = cams.squeeze(1)  # Remove channel dimension
