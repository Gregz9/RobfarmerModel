...
class FrameLSTM(nn.Module):
    ...
    def init_backbone(self, backbone):
        ...
        self.rnn = nn.LSTM(
            self.backbone.feat_dim + 4, self.hidden_size, batch_first=True
        )  # (B, T, num_maps)
        self.fc = nn.Linear(self.hidden_size, self.num_classes)

        feat_dim = self.backbone.feat_dim + 4
    ...
    def anticipation_loss(self, frame_feats, lstm_feats, batch):
        ...
        pos_hands, neg_hands = batch["pos_hands"], batch["neg_hands"]
        ...
        def embed(x, pos_hands):
            x = self.backbone(x)

            # Again, following Sigurds and Fillips approach (rnn_gazepoint)
            pos_hands1 = pos_hands[:, 0]
            pos_hands2 = pos_hands[:, 1]

            pos_hands_expanded1 = (
                pos_hands1.unsqueeze(2).unsqueeze(3).expand(-1, -1, 28, 28)
            )
            pos_hands_expanded2 = (
                pos_hands2.unsqueeze(2).unsqueeze(3).expand(-1, -1, 28, 28)
            )
            x_hands = torch.cat((x, pos_hands_expanded1, pos_hands_expanded2), dim=1)

            pred_frame = self.project(x_hands)
            pooled = self.pool(pred_frame).view(B, -1)
            return pooled

        positive_pooled = embed(positive, pos_hands)
        ...
        if self.ant_loss == "mse":
            ...
            neg_hands1 = neg_hands[:, 0]
            neg_hands2 = neg_hands[:, 1]
            neg_hands_expanded1 = (
                neg_hands1.unsqueeze(2).unsqueeze(3).expand(-1, -1, 28, 28)
            )
            neg_hands_expanded2 = (
                neg_hands2.unsqueeze(2).unsqueeze(3).expand(-1, -1, 28, 28)
            )
            negative_pooled = torch.cat(
                (negative_pooled, neg_hands_expanded1, neg_hands_expanded2), dim=1
            )
    ...
    def forward(self, batch, **kwargs):
        frames, length, hands_data = batch["frames"], batch["length"], batch["hands"]
        B, T = frames.shape[:2]
        S = self.spatial_dim
        # print(f"Type of hands: {type(hands_data)}")
        hands_data1 = (
            hands_data[:, :, 0::2, :]
            .transpose(-1, -2)
            .unsqueeze(-1)
        ...
        # print(hands_data.size())
        frame_feats = self.flatten_apply(
            frames, lambda t: self.backbone(t)
        )  # (B, T, 256, 28, 28)
        # print(f"frame feats size: {frame_feats.size()}")
        # Reshaping to match the shape out features from output by the dilated ResNet
        # gaze_feat = gaze_data.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, 28, 28)
        # print(f"gaze feat size: {gaze_feat.size()}")
        frame_feats = torch.cat((frame_feats, hands_data1, hands_data2), dim=2)
        # print(f"frame feats size: {frame_feats.size()}")
        pool_feats = self.flatten_apply(frame_feats, lambda t: self.pool(t)).view(
            B, T, -1
        )  # (B, T, 256)
        # print(f"pool feats size: {pool_feats.size()}")
    ...
    def ic_classifier(self, frame_feats, **kwargs):
        ...
        # Following Sigurds and Fillips approach, feeding a zero-vector for compatibility

        zero_tensor = torch.zeros_like(pool_feats)

        pool_feats = torch.cat((pool_feats, zero_tensor), dim=1)
...
def frame_lstm_hands(num_classes, max_len, backbone, hidden_size=2048, ant_loss="mse"):
