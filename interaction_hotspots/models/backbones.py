...
class DilatedResNetBackbone(nn.Module):
    def __init__(self, block, layers, strides=[1,2,2,2], dilations=[1,1,1,1], pretrained=None):
        ...
        self.lateral_1Dconv1 = nn.Conv2d(512, 512, kernel_size=1)
        self.lateral_1Dconv2 = nn.Conv2d(256, 256, kernel_size=1)
        self.lateral_1Dconv3 = nn.Conv2d(128, 128, kernel_size=1)
        self.lateral_1Dconv4 = nn.Conv2d(64, 64, kernel_size=1)
...
class FPNResNetBackBone(nn.Module):
    def __init__(self, block, layers, strides=[1,2,2,2], dilations=[1,1,1,1], pretrained=None):
        super().__init__()
        rnet = DilatedResNet(block, layers, strides, dilations)
