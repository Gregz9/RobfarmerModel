...
class FrameLSTM(nn.Module):
    ...
    def init_backbone(self, backbone):
        ...
        self.rnn = nn.LSTM(
            self.backbone.feat_dim + 2, self.hidden_size, batch_first=True
        )  # (B, T, num_maps)
    ...
    def anticipation_loss(self, frame_feats, lstm_feats, batch):
        ...
        def embed(x, pos_gaze):
            ...
            pos_gaze_expanded = (
                pos_gaze.unsqueeze(2).unsqueeze(3).expand(-1, -1, 28, 28)
            )
            x_gaze = torch.cat((x, pos_gaze_expanded), dim=1)

            pred_frame = self.project(x_gaze)
        ...
        if self.ant_loss == "mse":
            ...
            neg_gaze_expanded = (
                neg_gaze.unsqueeze(2).unsqueeze(3).expand(-1, -1, 28, 28)
            )
    ...
    def forward(self, batch, **kwargs):
        frames, length, gaze_data = batch["frames"], batch["length"], batch["gaze"]
        B, T = frames.shape[:2]
        S = self.spatial_dim
