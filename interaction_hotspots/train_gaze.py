...
checkpoint_path = f"/home/gregz9/Desktop/MasterThesis/data/checkpoints/{datetime.now().strftime('%d%m%Y%H%M')}_{args.max_epochs}_epochs_gaze_model"
metrics_path = f"/home/gregz9/Desktop/MasterThesis/data/training_metrics/"
loss_file = os.path.join(
    metrics_path,
    f"{datetime.now().strftime('%d%m%Y%H%M')}_{args.max_epochs}_epochs_gaze_model.json",
)
...
from interaction_hotspots.models import rnn, gaze_rnn, backbones

torch.backends.cudnn.enabled = False
net = gaze_rnn.frame_lstm_gaze(
    len(trainset.verbs),
    trainset.max_len,
...
log_path = os.path.join(
    ...
    f"gaze_model_{args.max_epochs}_{args.batch_size}_{args.lr}_{args.weight_decay}",
)
