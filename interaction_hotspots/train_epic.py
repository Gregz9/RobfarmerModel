...
net.load_state_dict(torch.load("cv/epic/ckpt_E_20.pth")["net"])
net.cuda()
...
for epoch in range(start_epoch, args.max_epochs + 1):
    ...
    try:
        subprocess.run(
            [
                "mkdir",
                f"/home/gregz9/Desktop/MasterThesis/src/interaction_hotspots/checkpoints/{datetime.now().strftime('%d%m%Y%H%M')}_{args.max_epochs}_epochs_base_model/base_model_{epoch}",
            ]
        )
    except:
    ...
    if epoch % 2 == 0:
        torch.save(
            ...
            f"/home/gregz9/Desktop/MasterThesis/src/interaction_hotspots/checkpoints/{datetime.now().strftime('%d%m%Y%H%M')}_{args.max_epochs}_epochs_base_model/base_model_{epoch}.pt",
        )
