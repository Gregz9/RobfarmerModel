...
def default_transform(split, size):
...
class PairedTransform:
    ...
    def train_transform(self, image, heatmap, **kwargs):
        ...
        assert (
            heatmap.shape[1],
            heatmap.shape[0],
        ) == image.size, "image and heatmap sizes mismatch (%s vs. %s)" % (
            image.size,
            heatmap.shape,
        )
    ...
    def val_transform(self, image, heatmap, **kwargs):

        # PIL and Tensor have inverted shapes
        # NOTE: Omitting assert, it breaks the pipeline
        # image.size == (606, 479) while heatmap.shape == (479, 606)
        # assert (
        #     heatmap.shape[1],
        #     heatmap.shape[0],
