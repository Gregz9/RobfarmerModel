...
def AUC_Judd(saliency_map, fixation_map, jitter=True):
    ...
    # print(f"Fixation map shape: {fixation_map.shape}")
    ...
    if saliency_map.shape != fixation_map.shape:
        # print("Saliency maps are getting reshaped")
    ...
    saliency_map = (saliency_map - np.min(saliency_map)) / (
        np.max(saliency_map) - np.min(saliency_map) + 1e-12
    )

    S = saliency_map.ravel()
    F = fixation_map.ravel()

    # print("Shape of maps after they're raveled")
    # print(f"Fixation map shape: {F.shape}")
    # print(f"Saliency map shape: {S.shape}")
...
class Evaluator:
    ...
    def score(self, pred, gt):
        ...
        gt_binary = (
            (gt_binary / gt_binary.max() + 1e-12) if gt_binary.max() > 0 else gt_binary
        )
        gt_binary = np.where(gt_binary > 0.5, 1, 0)

        # print("Printing shapes before AUC-J Calculations")
        # print(pred.shape)
    ...
    def batch_interp(self, inp, sz):
        ...
        for i in range(0, inp.shape[0], 512):
            chunk = inp[i : i + 512]
            out.append(
                F.interpolate(chunk, (sz, sz), mode="bilinear", align_corners=False)
            )
        out = torch.cat(out, 0)
        return out
    ...
    def evaluate(self, preds):
        ...
        # Adjustment of datetype of keys
        # print("Shape of predicitons")
        # print(np.array(list(self.gt.values())).shape)
        ...
        for key in ["KLD", "SIM", "AUC-J"]:
            ...
            mean, stderr = np.mean(key_score), np.std(key_score) / (
                np.sqrt(len(key_score))
            )
            log_str = "%s: %.3f ± %.3f (%d/%d)" % (
                key,
                mean,
