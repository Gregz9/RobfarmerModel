...
from models import rnn, cons_rnn, gaze_rnn, backbones, intcam
...
parser.add_argument("--model", type=str, default=224)
...
def visualize(path, viz_verbs, colors, sz=256):
    ...
    img = util.default_transform("val", sz)(img)
    ...
    uimg = F.interpolate(
        uimg.unsqueeze(0), (sz, sz), mode="bilinear", align_corners=False
    )[0]
...
# -----------------------------------------------------------------------------------------------------#
dataset = epic.EPICHeatmaps(root=data._DATA_ROOTS[args.dset], split="val", d_name=args.dset, size=args.resolution)
annot_file = open(os.path.join(data._DATA_ROOTS["Robofarmer-II"], "annotation.json"), "r")
...
net = net(len(viz_verbs), max_len=-1, backbone=backbones.dr50_n28)
...
for fl in glob.glob("%s/*.jpg" % args.inp):
    img = visualize(fl, viz_verbs, colors, args.resolution)
