...
from typing import Dict, Iterator, List, Tuple, cast, Optional

import numpy as np
from dataclasses import dataclass
import preprocess.types_pb2 as pb


# TODO: Update the list of __all__
__all__ = [
    "HandSide",
    "HandState",
    "HandLandmarkType",
...
class HandLandmark:
    x: np.float32
    y: np.float32
    l_type: HandLandmarkType

    def to_protobuf(self) -> pb.HandLandmark:
        landmark = pb.HandLandmark()
        landmark.x = self.x
        landmark.y = self.y
        landmark.landmark_type = self.l_type.value
        assert landmark.IsInitialized()
        return landmark

    @staticmethod
    def from_protobuf(landmark: pb.HandLandmark) -> "HandLandmark":
        return HandLandmark(x=landmark.x, y=landmark.y, l_type=landmark.l_type)

    @property
    def coords(self) -> Tuple[np.float32, np.float32]:
        return (self.x, self.y)

    @property
    def coords_int(self) -> Tuple[np.int32, np.int32]:
        return (np.int32(self.x), np.int32(self.y))

    def scale(self, height_factor: float = 1, width_factor: float = 1) -> None:
        self.x *= height_factor
        self.y *= width_factor
...
class HandPose:
    ...
    def to_protobuf(self) -> pb.HandPose:
        pose = pb.HandPose()
        pose.landmarks.extend([landmark.to_protobuf() for landmark in self.landmarks])
        assert pose.IsInitialized()
        return pose

    @staticmethod
    def from_protobuf(pose: pb.HandPose) -> "HandPose":
        return HandPose(
            landmarks=[
                HandLandmark.from_protobuf(pb_landmark)
                for pb_landmark in pose.landmarks
            ]
        )

    def scale(self, height_factor: float = 1, width_factor: float = 1) -> None:
        for landmark in self.landmarks:
            landmark.scale_point(height_factor, width_factor)
...
class GazePoint:
    ...
    def to_protobuf(self) -> pb.GazePoint:
        gaze_point = pb.GazePoint()
        gaze_point.x = self.x
        gaze_point.y = self.y
        assert gaze_point.IsInitialized()
        return gaze_point

    @staticmethod
    def from_protobuf(gaze_point: pb.GazePoint) -> "GazePoint":
        return GazePoint(
            x=gaze_point.x,
            y=gaze_point.y,
        )
    ...
    @property
    def coords_int(self) -> Tuple[int, int]:
...
class HandDetection:
    ...
    def to_protobuf(self) -> pb.HandDetection:
        ...
        # TODO: Fix the setting of pose
        # detection.pose = self.pose
...
class FrameDetections:
    ...
    def to_protobuf(self) -> pb.Detections:
        ...
        if self.gaze:
            detections.gaze.CopyFrom(self.gaze.to_protobuf())
        # detections.hand_poses.extend([pose.to_protobuf() for pose in self.hand_poses])
    ...
    def from_protobuf(detections: pb.Detections) -> "FrameDetections":
        return FrameDetections(
            ...
            gaze=(
                GazePoint.from_protobuf(detections.gaze)
                if hasattr(detections, "gaze")
                else GazePoint()
            ),
            # hand_poses=[HandPose.from_protobuf(pb) for pb in detections.hand_poses],
            # hand_poses=[],
        )
    ...
    def get_hand_object_interactions(
        ...
        for hand_idx, hand_detection in enumerate(self.hands):
            if (
                ...
                continue
            estimated_object_position = np.array(hand_detection.bbox.center) + np.array(
                hand_detection.object_offset.coord
            )
            distances = ((object_centers - estimated_object_position) ** 2).sum(axis=-1)
