...
def get_pair_homography(frame_1, frame_2, annot_1, annot_2, hand_threshold=0.1, obj_threshold=0.1):
    ...
    # plt.imshow(msk_img_1)
    # plt.show()
    # plt.imshow(msk_img_2)
    # plt.show()
    # img3 = cv2.drawMatches(frame_1, kpsA, frame_2, kpsB, matches[:100], outImg=None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
    # plt.imshow(img3)
    # plt.show()
...
def traj_compute(frames, annots, hand_sides, hand_threshold=0.1, obj_threshold=0.1):
    ...
    # print(f"TRAJ BEFORE VALIDATION: {left_traj}")
    left_traj = valid_traj(left_traj, imgW=imgW, imgH=imgH)
    # print(f"TRAJ AFTER VALIDATION: {left_traj}")
...
def traj_completion(traj, side, imgW=456, imgH=256):
    ...
    print(f"FULL TRAJECTORY: {full_traj}")
