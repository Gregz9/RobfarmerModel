...
def sample_action_anticipation_frames(frame_start, t_buffer=1, fps=4.0, fps_init=25.0):
...
def sample_anticipation_frames(frame_start, frame_stop, t_buffer=1, fps=4.0, fps_init=25.0): 
    num_frames = int(np.floor(t_buffer * fps))
    # frames = (np.arange(num_frames +1) - num_frames) / fps
    frames = np.arange(frame_start, frame_stop).astype(np.int32)
    stride = int(np.floor(len(frames) / fps))
